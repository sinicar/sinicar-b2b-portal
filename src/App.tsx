...
function AppContent() {
    ...
    const [authView, setAuthView] = useState<'LOGIN' | 'REGISTER' | 'PARTNER_REGISTER'>('LOGIN');
                    ...
                    if (session.expiresAt && session.expiresAt > Date.now() && session.sessionToken && session.userId) {
                        ...
                        if (MockApi.validateSessionToken(session.userId, session.sessionToken)) {
                            ...
                            if (user && user.isActive) {
                                ...
                                localStorage.setItem('b2b_session_sini_v2', JSON.stringify(user));
                                setCurrentUser(user);
                                setCurrentProfile(profile);
                                setShowIntro(false);
                                setLoading(false);
    ...
    if (authView === 'PARTNER_REGISTER') {
        return <PartnerRegister onBack={() => setAuthView('LOGIN')} />;
    }
                        ...
                        <div className="mt-10 pt-6 border-t border-white/5 flex flex-col items-center gap-3 text-center">
                            ...
                            {loginType === 'OWNER' && (
                                <>
                                    ...
                                    {/* زر كن شريكنا */}
                                    <button
                                        onClick={() => setAuthView('PARTNER_REGISTER')}
                                        ...
                                        data-testid="button-partner-register"
                                    >
