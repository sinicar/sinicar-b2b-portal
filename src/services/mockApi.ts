...
import { STORAGE_KEYS } from './mock-api/core/storage-keys';
import { DEFAULT_UI_TEXTS, DEFAULT_SETTINGS, INITIAL_PRODUCTS, INITIAL_BANNERS, INITIAL_NEWS, DEMO_ORDERS, DEMO_QUOTES } from './mock-api/core/defaults';
import { delay, generateClientId, generateActivationCode } from './mock-api/core/helpers/utils';
import { internalRecordActivity, logActivity } from './mock-api/core/helpers/activity';
import { internalCreateNotification, createEventNotification, getAdminUserIds } from './mock-api/core/helpers/notifications';

// Helper for daily reset (Search Points System)
const checkAndResetDailySearch = (user: User) => {
    const today = new Date().toISOString().split('T')[0];
    if (user.status === 'SUSPENDED') {
        // logic to check suspension date could go here
    }
...
export const MockApi = {
    ...
    // ==========================================
    // PARTNER REGISTRATION SYSTEM API
    // ==========================================

    // --- Create Partner Request ---
    async createPartnerRequest(data: any): Promise<any> {
        await delay(100);

        const partnerRequests: any[] = JSON.parse(localStorage.getItem('partner_requests') || '[]');
        ...
        localStorage.setItem('partner_requests', JSON.stringify(partnerRequests));

        // Log activity
        internalRecordActivity({
            userId: 'guest',
            userName: data.companyName || data.fullName || 'شريك جديد',
            role: 'GUEST',
            eventType: 'CREATE' as ActivityEventType,
            entityType: 'PARTNER_REQUEST' as EntityType,
            entityId: newRequest.id,
            description: `طلب شراكة جديد: ${data.type}`,
            metadata: { type: data.type, phone: data.phone }
        });
        ...
        adminUserIds.forEach(adminId => {
            const notification: Notification = {
                ...
                message: `طلب شراكة من ${typeLabels[data.type] || data.type}: ${data.companyName || data.fullName}`,
                isRead: false,
                createdAt: new Date().toISOString(),
                metadata: { partnerRequestId: newRequest.id, type: data.type }
            };
            notifications.push(notification);
        });
    ...
    // --- Check Partner Request Status ---
    async checkPartnerRequestStatus(phone: string, password: string): Promise<{ status: string; message: string }> {
        await delay(100);

        const partnerRequests: any[] = JSON.parse(localStorage.getItem('partner_requests') || '[]');
        const request = partnerRequests.find(r => r.phone === phone && r.password === password);
        ...
        const statusMessages: Record<string, { status: string; message: string }> = {
            NEW: { status: 'NEW', message: 'طلبك قيد الانتظار، سيتم مراجعته قريباً' },
            UNDER_REVIEW: { status: 'UNDER_REVIEW', message: 'طلبك قيد المراجعة من قبل فريق العمل' },
            APPROVED: { status: 'APPROVED', message: 'تمت الموافقة على طلبك! يمكنك الآن تسجيل الدخول' },
            REJECTED: { status: 'REJECTED', message: `تم رفض طلبك. السبب: ${request.rejectionReason || 'غير محدد'}` },
            NEEDS_MODIFICATION: { status: 'NEEDS_MODIFICATION', message: `طلبك يحتاج تعديل: ${request.modificationNote || 'الرجاء التواصل معنا'}` }
        };

        return statusMessages[request.status] || { status: request.status, message: 'حالة غير معروفة' };
    },

    // --- Get All Partner Requests (Admin) ---
    async getPartnerRequests(filters?: {
        type?: string;
        status?: string;
        search?: string;
        page?: number;
        ...
        let requests: any[] = JSON.parse(localStorage.getItem('partner_requests') || '[]');
    ...
    async getPartnerRequestById(id: string): Promise<any | null> {
        await delay(10);
        const requests: any[] = JSON.parse(localStorage.getItem('partner_requests') || '[]');
        return requests.find(r => r.id === id) || null;
    },
    ...
    async updatePartnerRequestStatus(
        ...
        requests[index] = {
            ...oldRequest,
            status,
            adminNotes: adminNotes || oldRequest.adminNotes,
            rejectionReason: rejectionReason || oldRequest.rejectionReason,
            modificationNote: modificationNote || oldRequest.modificationNote,
            reviewedAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        };
        ...
        internalRecordActivity({
            ...
            entityType: 'PARTNER_REQUEST' as EntityType,
            entityId: id,
            description: `تم تغيير حالة طلب الشراكة إلى: ${status}`,
    ...
    // --- Approve Partner Request and Create User ---
    async approvePartnerRequest(id: string, permissions: any): Promise<any> {
        await delay(100);

        const requests: any[] = JSON.parse(localStorage.getItem('partner_requests') || '[]');
        const requestIndex = requests.findIndex(r => r.id === id);

        if (requestIndex === -1) throw new Error('طلب غير موجود');
        ...
        // If local supplier, also create business profile
        if (request.type === 'LOCAL_SUPPLIER') {
            const profiles: BusinessProfile[] = JSON.parse(localStorage.getItem(STORAGE_KEYS.PROFILES) || '[]');
            const newProfile: any = {
                ...
                nationalAddress: request.nationalAddressNumber,
                phone: request.phone,
                status: 'ACTIVE',
                carBrands: request.carBrands || [],
                ownBrands: request.ownBrands || [],
                isSupplier: true,
                searchPointsBalance: permissions?.searchPoints || 100,
                createdAt: new Date().toISOString()
            };
            profiles.push(newProfile);
            localStorage.setItem(STORAGE_KEYS.PROFILES, JSON.stringify(profiles));
        ...
        internalRecordActivity({
            ...
            entityId: newUser.id,
            description: `تم الموافقة على طلب الشراكة وإنشاء حساب: ${newUser.name}`,
            metadata: { type: request.type, phone: request.phone }
        });

        return { user: newUser, request: requests[requestIndex] };
    },

    // --- Get Partner Requests Stats ---
    async getPartnerRequestsStats(): Promise<{
        total: number;
    ...
    async getOrderShortages(): Promise<any[]> {
        await delay(10);
        return JSON.parse(localStorage.getItem(STORAGE_KEYS.ORDER_SHORTAGES) || '[]');
    },

    async createOrderShortage(data: {
        orderId: string;
        orderItemId?: string;
        customerId: string;
        customerName: string;
        ...
        const shortages = JSON.parse(localStorage.getItem(STORAGE_KEYS.ORDER_SHORTAGES) || '[]');
        const now = new Date().toISOString();
        ...
        const newShortage = {
            ...
            statusHistory: [{
                ...
                changedAt: now,
                changedBy: 'System',
                notes: 'تم إنشاء النقص تلقائياً من الطلب'
            }],
            internalNotes: '',
            expectedDeliveryDate: new Date(Date.now() + data.deliveryHours * 60 * 60 * 1000).toISOString(),
            isNew: true,
            createdAt: now,
            updatedAt: now
        };

        shortages.push(newShortage);
        localStorage.setItem(STORAGE_KEYS.ORDER_SHORTAGES, JSON.stringify(shortages));
        ...
        internalCreateNotification({
            ...
            type: 'SYSTEM' as NotificationType,
            title: 'طلبية جديدة',
            message: `طلبية جديدة للعميل ${data.customerName}: ${data.productName}`,
    ...
    async updateOrderShortageStatus(
        shortageId: string,
        newStatus: string,
        notes?: string,
        ...
        const shortages = JSON.parse(localStorage.getItem(STORAGE_KEYS.ORDER_SHORTAGES) || '[]');
        const idx = shortages.findIndex((s: any) => s.id === shortageId);
        ...
        shortage.isNew = false;

        // Add to history
        shortage.statusHistory.push({
            status: newStatus,
            changedAt: now,
            changedBy,
            notes
        ...
        if (newStatus === 'DELIVERED') {
            shortage.actualDeliveryDate = now;
        }

        shortages[idx] = shortage;
        localStorage.setItem(STORAGE_KEYS.ORDER_SHORTAGES, JSON.stringify(shortages));
        ...
        internalRecordActivity({
            ...
            entityId: shortageId,
            description: `تم تحديث حالة الطلبية إلى ${newStatus}`,
            metadata: { shortageId, newStatus, notes }
        });

        return true;
    },

    async getOrderShortagesStats(): Promise<{
        total: number;
        new: number;
        contacting: number;
        preparing: number;
        ...
        const shortages = await this.getOrderShortages();
        return {
            total: shortages.length,
            new: shortages.filter((s: any) => s.status === 'NEW').length,
            contacting: shortages.filter((s: any) => s.status === 'CONTACTING').length,
            preparing: shortages.filter((s: any) => s.status === 'PREPARING' || s.status === 'CONFIRMED').length,
            received: shortages.filter((s: any) => s.status === 'RECEIVED').length,
            delivered: shortages.filter((s: any) => s.status === 'DELIVERED').length,
            cancelled: shortages.filter((s: any) => s.status === 'CANCELLED').length
        };
    },

    async markOrderShortagesAsSeen(): Promise<void> {
        const shortages = JSON.parse(localStorage.getItem(STORAGE_KEYS.ORDER_SHORTAGES) || '[]');
        shortages.forEach((s: any) => { s.isNew = false; });
        localStorage.setItem(STORAGE_KEYS.ORDER_SHORTAGES, JSON.stringify(shortages));
    }
};
